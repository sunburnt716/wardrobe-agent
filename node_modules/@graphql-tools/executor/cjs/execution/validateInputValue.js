...
const didYouMean_js_1 = require("./didYouMean.js");
const suggestionList_js_1 = require("./suggestionList.js");
/**
...
function validateInputValueImpl(inputValue, type, onError, hideSuggestions, path) {
    if ((0, graphql_1.isNonNullType)(type)) {
        if (inputValue === undefined) {
            reportInvalidValue(onError, `Expected a value of non-null type "${type}" to be provided.`, path, inputValue);
            return;
        }
        if (inputValue === null) {
            reportInvalidValue(onError, `Expected value of non-null type "${type}" not to be null.`, path, inputValue);
            return;
        }
    ...
    if ((0, graphql_1.isListType)(type)) {
        if (!(0, utils_1.isIterableObject)(inputValue)) {
            ...
            for (const itemValue of inputValue) {
                validateInputValueImpl(itemValue, type.ofType, onError, hideSuggestions, (0, utils_1.addPath)(path, index++, undefined));
            }
        }
    }
    else if ((0, graphql_1.isInputObjectType)(type)) {
        if (!(0, utils_1.isObjectLike)(inputValue) || Array.isArray(inputValue)) {
            reportInvalidValue(onError, `Expected value of type "${type}" to be an object, found: ${(0, utils_1.inspect)(inputValue)}.`, path, inputValue);
            return;
        }
        ...
        for (const field of Object.values(fieldDefs)) {
            ...
            if (fieldValue === undefined) {
                if ((0, graphql_1.isRequiredInputField)(field)) {
                    reportInvalidValue(onError, `Expected value of type "${type}" to include required field "${field.name}", found: ${(0, utils_1.inspect)(inputValue)}.`, path, inputValue);
                }
            }
            else {
                validateInputValueImpl(fieldValue, field.type, onError, hideSuggestions, (0, utils_1.addPath)(path, field.name, type.name));
            }
        }
        const fields = [];
        ...
        for (const fieldName of Object.keys(inputValue)) {
            ...
            if (!Object.hasOwn(fieldDefs, fieldName)) {
                ...
                reportInvalidValue(onError, `Expected value of type "${type}" not to include unknown field "${fieldName}"${suggestion ? `.${suggestion} Found` : ', found'}: ${(0, utils_1.inspect)(inputValue)}.`, path, inputValue);
                continue;
            }
        ...
        if (type.isOneOf) {
            ...
            if (value === null) {
                reportInvalidValue(onError, getOneOfInputObjectErrorMessage(type), (0, utils_1.addPath)(path, field, type.name), inputValue);
            }
        }
    }
    else {
        (0, graphql_1.assertLeafType)(type);
        let result;
        let caughtError;
        try {
            const typeAny = type;
            const methodName = graphql_1.versionInfo.major >= 17 ? 'coerceInputValue' : 'parseValue';
            result = typeAny[methodName](inputValue);
        }
        catch (error) {
            if (error instanceof graphql_1.GraphQLError) {
                onError((0, utils_1.pathToArray)(path), inputValue, error);
                return;
            }
            caughtError = error;
        }
        if (result === undefined) {
            reportInvalidValue(onError, `Expected value of type "${type}"${caughtError != null
                ...
                : ', found'}: ${(0, utils_1.inspect)(inputValue)}.`, path, inputValue, caughtError);
        }
    }
}
function reportInvalidValue(onError, message, path, invalidValue, originalError) {
    onError((0, utils_1.pathToArray)(path), invalidValue, (0, utils_1.createGraphQLError)(message, { originalError }));
}
/**
...
function validateInputLiteralImpl(context, valueNode, type, hideSuggestions, path) {
    ...
    if ((0, graphql_1.isNonNullType)(type)) {
        if (valueNode.kind === graphql_1.Kind.NULL) {
    ...
    if ((0, graphql_1.isListType)(type)) {
        if (valueNode.kind !== graphql_1.Kind.LIST) {
            ...
            for (const itemNode of valueNode.values) {
                validateInputLiteralImpl(context, itemNode, type.ofType, hideSuggestions, (0, utils_1.addPath)(path, index++, undefined));
            }
        }
    }
    else if ((0, graphql_1.isInputObjectType)(type)) {
        if (valueNode.kind !== graphql_1.Kind.OBJECT) {
            reportInvalidLiteral(context.onError, `Expected value of type "${type}" to be an object, found: ${(0, graphql_1.print)(valueNode)}.`, valueNode, path);
            return;
        }
        const fieldDefs = type.getFields();
        ...
        for (const field of Object.values(fieldDefs)) {
            ...
            if (fieldNode === undefined) {
                if ((0, graphql_1.isRequiredInputField)(field)) {
                    reportInvalidLiteral(context.onError, `Expected value of type "${type}" to include required field "${field.name}", found: ${(0, graphql_1.print)(valueNode)}.`, valueNode, path);
                }
            }
            else {
                const fieldValueNode = fieldNode.value;
                if (fieldValueNode.kind === graphql_1.Kind.VARIABLE && !context.static) {
                    ...
                    else if (value === undefined && !(0, graphql_1.isRequiredInputField)(field)) {
                        continue;
                    }
                }
                validateInputLiteralImpl(context, fieldValueNode, field.type, hideSuggestions, (0, utils_1.addPath)(path, field.name, type.name));
            }
        }
        ...
        for (const fieldNode of fields) {
            ...
            if (!Object.hasOwn(fieldDefs, fieldName)) {
                const suggestion = hideSuggestions
                    ? ''
                    : (0, didYouMean_js_1.didYouMean)((0, suggestionList_js_1.suggestionList)(fieldName, Object.keys(fieldDefs)));
                reportInvalidLiteral(context.onError, `Expected value of type "${type}" not to include unknown field "${fieldName}"${suggestion ? `.${suggestion} Found` : ', found'}: ${(0, graphql_1.print)(valueNode)}.`, fieldNode, path);
            }
            else {
        ...
        if (type.isOneOf) {
            ...
            if (fieldValueNode.kind === graphql_1.Kind.NULL) {
                const fieldName = knownFields[0].name.value;
                reportInvalidLiteral(context.onError, getOneOfInputObjectErrorMessage(type), valueNode, (0, utils_1.addPath)(path, fieldName, undefined));
            }
        }
    }
    else {
        ...
        try {
            ...
            if (error instanceof graphql_1.GraphQLError) {
                context.onError(error, (0, utils_1.pathToArray)(path));
                return;
            }
            caughtError = error;
        }
        if (result === undefined) {
            reportInvalidLiteral(context.onError, `Expected value of type "${type}"${caughtError != null
                ? `, but encountered error "${getCaughtErrorMessage(caughtError)}"; found`
                : ', found'}: ${(0, graphql_1.print)(valueNode)}.`, valueNode, path, caughtError);
        }
    }
}
...
function reportInvalidLiteral(onError, message, valueNode, path, originalError) {
    onError((0, utils_1.createGraphQLError)(message, {
        nodes: valueNode,
        originalError,
    }), (0, utils_1.pathToArray)(path));
}
function getCaughtErrorMessage(caughtError) {
    if ((0, utils_1.isObjectLike)(caughtError)) {
