...
function realisticDelay({ min = 20, max = 50, } = {}) {
    (0, invariant_1.invariant)(max > min, 23);
...
class MockLink extends link_1.ApolloLink {
    ...
    getMockedResponses(request) {
        const key = JSON.stringify({
            query: (0, utilities_1.print)(removeAutoDeferLabels((0, utilities_1.addTypenameToDocument)(request.query))),
        });
...
function removeAutoDeferLabels(document) {
        ...
        Directive(node) {
            ...
            return {
                ...
                arguments: node.arguments?.filter((arg) => {
                    return (arg.name.value !== "label" ||
                        arg.value.kind !== graphql_1.Kind.STRING || !arg.value.value.match(/^ac_\d+$/));
                }),
            };
        },
    });
