...
function shouldInclude({ directives }, variables) {
    ...
    return getInclusionDirectives(directives).every(({ directive, ifArgument }) => {
        ...
        if (ifArgument.value.kind === "Variable") {
            ...
            (0, invariant_1.invariant)(evaledValue !== void 0, 17, directive.name.value);
        }
        else {
...
function getInclusionDirectives(directives) {
    ...
    if (directives && directives.length) {
        directives.forEach((directive) => {
            ...
            (0, invariant_1.invariant)(directiveArguments && directiveArguments.length === 1, 18, directiveName);
            const ifArgument = directiveArguments[0];
            (0, invariant_1.invariant)(ifArgument.name && ifArgument.name.value === "if", 19, directiveName);
