...
class ObservableQuery {
    ...
    constructor({ queryManager, options, transformedQuery = queryManager.transform(options.query), }) {
        ...
        if (options[internal_1.variablesUnknownSymbol]) {
            (0, invariant_1.invariant)(fetchPolicy === "standby", 84);
    ...
    */
    ...
    refetch(variables) {
        ...
        if (environment_1.__DEV__ && variables && hasOwnProperty.call(variables, "variables")) {
            ...
            if (!vars || !vars.some((v) => v.variable.name.value === "variables")) {
                __DEV__ && invariant_1.invariant.warn(85, variables, queryDef.name?.value || queryDef);
            }
        }
    ...
    fetchMore({ query, variables, context, errorPolicy, updateQuery, }) {
        ...
        if (!isCached) {
            (0, invariant_1.invariant)(updateQuery, 87);
        }
            ...
            next: (subscriptionData) => {
                ...
                if (error) {
                    if (onError) {
                        ...
                        invariant_1.invariant.error(88, error);
                    }
                    return;
                }
                if (updateQuery) {
        ...
        if (shouldCancelPolling()) {
            if (environment_1.__DEV__) {
                if (!this.didWarnCacheOnlyPolling &&
                    ...
                    __DEV__ && invariant_1.invariant.warn(89, (0, internal_1.getOperationName)(this.query, "(anonymous)"));
...
function logMissingFieldErrors(missing) {
    if (environment_1.__DEV__ && missing) {
        __DEV__ && invariant_1.invariant.debug(90, missing);
    }
}
